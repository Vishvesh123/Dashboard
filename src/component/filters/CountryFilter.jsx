...
function CountryFilter(props) {
  const countryData_india = props.data.filter(
    (item) => item.country === "India"
  );

  //Bar Chart
  const likelihoodValues = countryData_india.map((item) => item.likelihood);
  const options = {
    ...
    plugins: {
      title: {
        text: " The Likelihood of different topic for the Country India",
      },
    },
  };

  const barChart = {
    labels: countryData_india.map((item) => item.topic),
  ...
  //Pie Chart
  const intensityValues = countryData_india.map((item) => item.intensity);
  const pieChart = {
    labels: countryData_india.map((item) => item.topic),
    datasets: [
      {
  ...
  return (
    <div className={`container-filter${props.isVisible ? " slide-menu2" : ""}`}>
      <div style={{ marginTop: "80px" }} className="Upper-Chart">
        {props.countryDataArray.map((item) => (
          <div key={item.country} className="chart">
            <h3>{item.country}</h3>
